...
export function ProjectCard({ project, readLabel = "Lihat proyek", homeMotion = false }: { project: Project; readLabel?: string; homeMotion?: boolean }) {
  return <article className={`project-card accent-${project.accent}`} data-reveal={homeMotion ? undefined : true} data-home-card={homeMotion ? true : undefined}>
    <Link href={`/proyek/${project.slug}`} className="project-card-link">
      <div className={`project-visual${project.heroImage ? " has-image" : ""}`} data-home-image={homeMotion ? true : undefined} data-motion-media={homeMotion ? undefined : true}>{project.heroImage ? <Image src={project.heroImage.url} alt={project.heroImage.alt} fill sizes="(max-width: 800px) 100vw, 33vw" data-home-image-media={homeMotion ? true : undefined} /> : <><span>{project.index}</span><i /></>}</div>
