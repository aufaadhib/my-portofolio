    ...
    () => {
      ...
      media.add(
        ...
        (context) => {
          ...
          const revealPage = () => {
            if (started) return;
            const { intro, reveals, rules, mediaImages, cardGroups, serviceGroups, staggerGroups } =
              collect();
            ...
            if (reveals.length)
              gsap.set(reveals, { y: desktop ? 62 : 34, autoAlpha: 0, force3D: true });
            if (rules.length)
              gsap.set(rules, { scaleX: 0, transformOrigin: "0% 50%", force3D: true });
            if (mediaImages.length)
              gsap.set(mediaImages, { scale: 1.045, transformOrigin: "50% 50%", force3D: true });
            ...
            if (rules.length)
              timeline.to(
                rules,
                { scaleX: 1, duration: 0.95, ease: "power3.inOut", clearProps: "transform" },
                intro.length ? "-=.9" : 0,
              );
            ...
            serviceGroups.forEach((group) =>
              animateGroup(
                group,
                gsap.utils.toArray<HTMLElement>("[data-reveal]", group),
                { x: desktop ? 46 : 0, y: desktop ? 0 : 30, autoAlpha: 0 },
                {
                  x: 0,
                  y: 0,
                  autoAlpha: 1,
            ...
            staggerGroups.forEach((group) =>
              animateGroup(
                group,
                gsap.utils.toArray<HTMLElement>(":scope > [data-reveal]", group),
                { y: desktop ? 74 : 38, autoAlpha: 0 },
                {
                  y: 0,
                  autoAlpha: 1,
                  duration: desktop ? 1.05 : 0.76,
                  stagger: 0.1,
                  ease: "power3.out",
                  overwrite: "auto",
                  clearProps: "transform,opacity,visibility",
                },
                desktop ? "top 78%" : "top 86%",
              ),
            );
            reveals
              .filter((target) => !grouped.has(target))
              .forEach((target) =>
                gsap.to(target, {
                  y: 0,
                  autoAlpha: 1,
            ...
            mediaImages.forEach((image) => {
              if (image.parentElement)
                gsap.to(image, {
                  ...
                  scrollTrigger: { trigger: image.parentElement, start: "top 88%", once: true },
                });
            });
          ...
          if (
            window.__portfolioRevealPending ||
            !overlay ||
