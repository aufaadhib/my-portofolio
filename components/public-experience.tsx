...
export function PublicExperience({ children, locale }: { children: ReactNode; locale: Locale }) {
  ...
  useEffect(() => {
    ...
    const handleClick = (event: globalThis.MouseEvent) => {
      ...
      if (url.origin !== window.location.origin || url.pathname.startsWith("/admin") || url.pathname === pathname || url.hash) return;
  ...
  useGSAP(() => () => { if (safetyTimer.current) clearTimeout(safetyTimer.current); }, []);
...
export function TransitionLink({ href, onClick, ...props }: LinkProps & Omit<React.AnchorHTMLAttributes<HTMLAnchorElement>, keyof LinkProps>) {
  const navigate = useContext(TransitionContext);
  const target = typeof href === "string" ? href : href.pathname ?? "";
  function handleClick(event: MouseEvent<HTMLAnchorElement>) {
    onClick?.(event);
    if (event.defaultPrevented || event.button !== 0 || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey || props.target === "_blank" || !target.startsWith("/") || target.startsWith("/admin") || target.includes("#")) return;
    event.preventDefault();
    navigate(target);
  }
