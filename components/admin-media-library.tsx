...
export function AdminMediaLibrary({
  ...
  assets: { id: string; url: string; alt: string; pathname: string; size: number }[];
}) {
  ...
  async function upload(form: FormData) {
    ...
    try {
      ...
      if (response.ok) {
        (document.getElementById("media-upload") as HTMLFormElement)?.reset();
        router.refresh();
      }
    } catch {
  ...
  async function remove(id: string, alt: string) {
    ...
    try {
      ...
      setMessage("Media gagal dihapus. Periksa koneksi lalu coba lagi.");
    } finally {
      busyRef.current = false;
      setBusyAction(null);
      setDeletingId(null);
  ...
  return (
    <>
      <form
        id="media-upload"
        className="admin-callout admin-editor"
      ...
      >
        ...
        <label>
          Alt text
          <input name="alt" required maxLength={240} disabled={busy} />
        </label>
      ...
      <div className="admin-media-grid">
        {assets.map((asset) => (
          <article key={asset.id}>
            <Image src={asset.url} alt={asset.alt} width={480} height={360} unoptimized />
            ...
            <button
              ...
              type="button"
              disabled={busy}
              onClick={() => remove(asset.id, asset.alt)}
            >
              {deletingId === asset.id ? (
                <>
                  <span className="admin-spinner" aria-hidden="true" />
