...
export async function DELETE(request: Request) {
  try {
    ...
    const asset = await db.mediaAsset.findUnique({ where: { id }, select: { id: true, url: true, pathname: true } });
    if (!asset) return NextResponse.json({ error: "Media tidak ditemukan" }, { status: 404 });
    const activeRevisions = await db.contentRevision.findMany({ where: { status: { in: ["DRAFT", "PUBLISHED"] } }, select: { payload: true, document: { select: { kind: true, slug: true } } } });
    const usedBy = activeRevisions.filter((revision) => JSON.stringify(revision.payload).includes(`"${id}"`)).map((revision) => `${revision.document.kind.toLowerCase()} ${revision.document.slug ?? "utama"}`);
    if (usedBy.length) return NextResponse.json({ error: `Media masih digunakan oleh ${usedBy.join(", ")}. Lepaskan dari konten lalu simpan terlebih dahulu.` }, { status: 409 });
    await db.$transaction(async (tx) => {
      await tx.mediaAsset.delete({ where: { id } });
      await tx.auditLog.create({ data: { actorId: owner.id, action: "delete_media", entityType: "MediaAsset", entityId: id, metadata: { pathname: asset.pathname } } });
    });
    await del(asset.url);
