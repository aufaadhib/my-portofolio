...
export default async function ServicesPage() {
  ...
  const { services } = await getPublicContent(locale);
  ...
  return (
    <>
      <main className="page-shell interior-page">
        ...
        <div className="service-list service-list-page">
          {services.map((service) => (
            <article key={service.index} className="service-row" data-reveal>
              <span className="eyebrow">{service.index}</span>
              <h2>{service.title}</h2>
              <p>{service.description}</p>
              <span className="placeholder-note">{t.note}</span>
            </article>
          ))}
        </div>
      </main>
