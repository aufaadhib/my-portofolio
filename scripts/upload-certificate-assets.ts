...
async function uploadCertificateAssets() {
  ...
  if (!connectionString || !token || !email)
    throw new Error("DATABASE_URL, BLOB_READ_WRITE_TOKEN, and CMS_OWNER_EMAIL are required");
  ...
  try {
    ...
    for (const certificate of fallbackCertificates) {
      ...
      const pdfBody = await readFile(
        path.join(process.cwd(), "public", decodeURIComponent(certificate.document)),
      );
      const [imageBlob, pdfBlob] = await Promise.all([
        put(`portfolio/certificates/${slug}.jpg`, imageBody, {
          access: "public",
        ...
        put(`portfolio/certificates/${slug}.pdf`, pdfBody, {
          access: "public",
          addRandomSuffix: false,
          allowOverwrite: true,
      ...
      const [imageAsset, pdfAsset] = await Promise.all([
        prisma.mediaAsset.upsert({
          where: { pathname: imageBlob.pathname },
          update: {
            url: imageBlob.url,
        ...
        prisma.mediaAsset.upsert({
          where: { pathname: pdfBlob.pathname },
          update: {
            url: pdfBlob.url,
            alt: `${certificate.title} PDF`,
      ...
      const document = await prisma.contentDocument.findUniqueOrThrow({
        where: { kind_slug: { kind: "CERTIFICATE", slug } },
      });
      ...
      if (current.imageMediaId === imageAsset.id && current.credentialMediaId === pdfAsset.id)
        continue;
      await prisma.$transaction(async (tx) => {
        ...
        const revision = await tx.contentRevision.create({
          data: {
            ...
            payload: {
              ...current,
              imageMediaId: imageAsset.id,
              credentialMediaId: pdfAsset.id,
              imageUrl: "",
              credentialUrl: pdfBlob.url,
            },
            createdById: owner.id,
        ...
        await tx.auditLog.create({
          data: {
            actorId: owner.id,
            action: "upload_certificate_assets",
...
uploadCertificateAssets().catch((error) => {
  console.error(error instanceof Error ? error.message : "Certificate upload failed");
  process.exitCode = 1;
});
