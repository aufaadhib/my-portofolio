...
async function bootstrapOwner() {
  ...
  if (!connectionString || !email || !password)
    throw new Error("DATABASE_URL, CMS_OWNER_EMAIL, and CMS_OWNER_PASSWORD are required");
  if (password.length < 12)
    throw new Error("CMS_OWNER_PASSWORD must contain at least 12 characters");
  ...
  try {
    ...
    await prisma.$transaction([
      prisma.user.create({
        data: { id: userId, name: "Farhan Aufa Adhib", email, emailVerified: true, role: "owner" },
      }),
