...
async function publishProject() {
  ...
  try {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      await tx.auditLog.create({ data: { actorId: owner.id, action: "publish_project", entityType: "ContentRevision", entityId: revision.id, metadata: { kind: "PROJECT", slug: payload.slug, source: "https://rsudblambangan.id" } } });
      return { documentId: document.id, revisionId: revision.id, version: revision.version, unchanged: false };
    });
  } finally { await prisma.$disconnect(); }
}

publishProject().then((result) => console.log(JSON.stringify(result))).catch((error) => { console.error(error); process.exitCode = 1; });
