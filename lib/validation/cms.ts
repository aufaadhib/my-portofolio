...
const projectTranslationSchema = z.object({
  ...
  body: z
    .array(
      z.object({
        type: z.enum(["paragraph", "heading", "list", "quote", "image", "link"]),
...
export const projectPayloadSchema = z.object({
  ...
  content: z.object({ id: projectTranslationSchema, en: projectTranslationSchema }),
  year: z.string().max(20),
  stack: z.array(z.string().min(1).max(60)).max(20),
  featured: z.boolean(),
  sortOrder: z.number().int(),
  heroMediaId: z.string().nullable(),
  galleryMediaIds: z.array(z.string()).max(30),
...
const serviceTranslationSchema = z.object({
  ...
  scope: z.array(z.string().max(300)).max(30),
  deliverables: z.array(z.string().max(300)).max(30),
});
export const servicePayloadSchema = z.object({
  index: z.string().max(20),
  content: z.object({ id: serviceTranslationSchema, en: serviceTranslationSchema }),
  sortOrder: z.number().int(),
  visible: z.boolean(),
});
const resourceUrlSchema = z
  ...
  .refine(
    (value) =>
      !value ||
      value.startsWith("/certificate/") ||
      (URL.canParse(value) && new URL(value).protocol === "https:"),
    "Gunakan URL HTTPS atau path /certificate/",
  );
const certificateTranslationSchema = z.object({
  title: z.string().min(1).max(200),
  issuer: z.string().max(160),
});
export const certificatePayloadSchema = z
  .object({
    content: z.object({ id: certificateTranslationSchema, en: certificateTranslationSchema }),
    year: z.string().max(20),
    imageMediaId: z.string().nullable(),
    credentialMediaId: z.string().nullable(),
    imageUrl: resourceUrlSchema,
    credentialUrl: resourceUrlSchema,
    sortOrder: z.number().int(),
    visible: z.boolean(),
  })
  .refine((value) => value.imageMediaId || value.imageUrl, {
    message: "Pilih media atau isi path gambar",
  });
export const settingsPayloadSchema = z.object({
  siteName: z.string().min(1).max(120),
  siteUrl: z.url(),
  defaultTitle: z.string().max(160),
  defaultDescription: z.string().max(320),
  ogImageMediaId: z.string().nullable(),
  copyrightText: z.string().max(160),
...
export const payloadSchemas = {
  ...
  CERTIFICATE: certificatePayloadSchema,
