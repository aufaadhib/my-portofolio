...
export async function publishRevision(documentId: string, revisionId: string, kind: CmsKind, payload: unknown) {
  ...
  revalidatePath("/"); revalidatePath("/proyek"); revalidatePath("/tentang"); revalidatePath("/layanan"); revalidatePath("/kontak"); revalidatePath("/sitemap.xml");
}
